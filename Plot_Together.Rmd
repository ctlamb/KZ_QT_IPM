...
load(here::here("KZ", "out_rnd_eff.Rdata"))
kz <- out_rnd_eff
...
load(here::here("QT", "out_rnd_eff.Rdata"))
qt <- out_rnd_eff
...
ggplot(fit_df ,aes(x = yrs, y = est, ymin=q2.5, ymax=q97.5, fill=type)) +
...
##KZ EFFECT
```{r wolf, fig.height=5, fig.width=11, message=FALSE, warning=FALSE, results='hide'}
S <- ggs(kz$samples)%>%
...
```{r Summarise growth rates, results='asis'}
