...
kz.raw <- read_excel("/Users/clayton.lamb/Dropbox/Caribou/JWM_Klinse-za demographics/Analysis/Klinse-zaEackerInputs_200607_CL.xlsx",
                      sheet="SurvivalKlinse-zaSPenFre")%>%
  mutate(dur=exit-enter)

qt.raw <- read_excel("/Users/clayton.lamb/Dropbox/Caribou/JWM_Klinse-za demographics/Analysis/QuintetteEackerInputs_200710_CL.xlsx",
                      sheet="QuintetteSPooled")%>%
  mutate(dur=exit-enter)


kz.raw%>%
  filter(Year<=2013)%>%
  summarise(an.yrs=sum(dur)/13, ##these were all 12
            animals=n_distinct(id))

kz.raw%>%
  filter(Year>2013)%>%
  summarise(an.yrs=sum(dur)/13,
            animals=n_distinct(id))
...
qt.raw%>%select(id,Year,dur)%>%rbind(kz.raw%>%select(id,Year,dur))%>%
    summarise(an.yrs=sum(dur)/13,
            animals=n_distinct(id))
