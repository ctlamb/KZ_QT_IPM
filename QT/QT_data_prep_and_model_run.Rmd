...
count_dat <- read_xlsx(here::here("data", "QT", "Count_summary_QT.xlsx"))%>%
  mutate_at(c("SurveryCount_ADULTMF","SurveryCount_CALFMF", "Estimate_ADULTMF","Estimate_CALFMF"), funs(round(., 0)))%>%
    mutate(SurveryCount_ADULTMF=case_when(SurveryCount_ADULTMF<MinCount_ADULTMF~MinCount_ADULTMF, TRUE~SurveryCount_ADULTMF),
         SurveryCount_CALFMF=case_when(SurveryCount_CALFMF<MinCount_CALFMF~MinCount_CALFMF, TRUE~SurveryCount_CALFMF),
         Estimate_ADULTMF=case_when(Estimate_ADULTMF<MinCount_ADULTMF~MinCount_ADULTMF, TRUE~Estimate_ADULTMF),
         Estimate_CALFMF=case_when(Estimate_CALFMF<MinCount_CALFMF~MinCount_CALFMF, TRUE~Estimate_CALFMF))
...
# ##significant evidence that this is a poor estimate of population abundance
# ##min count was 34% > than the sightability corrected survey count.
count_dat[21,"Estimate_ADULTMF"] <- NA
count_dat[21,"SD_Estimate_ADULTMF"] <- NA
count_dat[21,"Estimate_CALFMF"] <- NA
count_dat[21,"SD_Estimate_CALFMF"] <- NA
# 
# count_dat[21,"SurveryCount_ADULTMF"] <- NA
# count_dat[21,"SurveryCount_CALFMF"] <- NA
...
```{r sight bootstrap, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, results='hide'}
...
##MODEL DIAGNOSTICS
```{r MODEL DIAGNOSTICS,eval=FALSE, results='hide', message=FALSE, warning=FALSE}
