...
rmarkdown::render(here::here("GM",'GM_dat_prep_and_model_run.Rmd'),
                  output_file = "README.md")

knitr::purl(input=here::here("GM",'GM_dat_prep_and_model_run.Rmd'),
            output=here::here("GM",'GM_dat_prep_and_model_run.R'))
```
...
# Read in raw data
adult_female_survival <- read.csv(here::here("data", "GM", "adult_female_survival_GM.csv"))
adult_female_recruit <- read.csv(here::here("data", "GM", "adult_female_recruit_GM.csv"))

adult_sex_ratio <- read.csv(here::here("data", "GM", "adult_sexratio_GM.csv"))

adult_female_mincount <- read.csv(here::here("data", "GM", "adult_female_mincount_GM.csv"))
calf_mincount <- read.csv(here::here("data", "GM", "calf_mincount_GM.csv"))
...
model_parms <- c("lambda","logla",
	...
	"diff_geom_mean_lambda_post_to_pre", "geom_mean_lambda_post_iwolf",
...
out_rnd_eff <- jagsUI::jags(ipm_dat, 
		inits = ipm_inits,
		model_parms,
		model.file = here::here("jags","GM_IPM_rnd_eff_CL.txt"),
		n.chains = nc, 
		n.iter = ni,
		n.burnin = nb,
		n.thin = nt,
...
## Plot results
```{r Plot results-lambda, results='hide', message=FALSE, warning=FALSE, eval=FALSE, include=FALSE}
###LAMBDA


#  Run model with survival and recruitment varying pre and post pen/wolf
#  Adjust n.cores for your computer
out_wolfpen_eff <- jagsUI::jags(ipm_dat, 
		inits = ipm_inits,
		model_parms,
		model.file = here::here("jags", "KZ_IPM_wolfpen_eff.txt"),
		n.chains = nc, 
		n.iter = ni,
		n.burnin = nb,
		n.thin = nt,
...
#save(out_wolfpen_eff, file = "out_wolfpen_eff.Rdata")
...
lambda_per_year_rnd_eff_mod <- out_rnd_eff$mean$lambda
lambda_per_year_wolfpen_eff_mod <- out_wolfpen_eff$mean$lambda


res_df <- as.data.frame(cbind(yr_df, 
	lambda_rnd_eff_mod = lambda_per_year_rnd_eff_mod,
	lambda_wolfpen_eff_mod = lambda_per_year_wolfpen_eff_mod)) %>%
	gather("model", "lambda", -yr_idx, -yrs)

ggplot(res_df) +
	geom_point(aes(x = yrs, y = lambda, group = model, colour = model)) +
	geom_smooth(aes(x = yrs, y = lambda, group = model, colour = model), size = 1.5) +
	scale_colour_manual(name = "Model", 
		values = c(wes_palette("Zissou1")[5], wes_palette("Zissou1")[4]),
		labels = c("Annually-varying S and R", "Treatment-varying S and R"))+
  theme_ipsum()+
  geom_hline(yintercept = 1, linetype="dashed")+
  labs(x="Year", y="Population growth", title="Population Growth by Model Structure")
```
...
raw.abund <-rbind(adult_female_mincount%>%mutate(param="MinCount_F",
  ...
  select(colnames(calc.abund)),  ###ADD AF Count
        ...
        calf_mincount%>%mutate(param="MinCount_Calf",
                                         ...
                                         yrs=Year,
                                         yr_idx=NA)%>%
  select(colnames(calc.abund))  ###ADD CALVES
  )%>%
  group_by(yrs,yr_idx)%>%
...
out_rnd_eff$q2.5$diff_geom_mean_lambda_post_to_pre

out_rnd_eff$mean$lambda[19]
out_rnd_eff$q2.5$lambda[19]
out_rnd_eff$q97.5$lambda[19]
```
