---
title: "Nutritional Hyopothesis: Klinse-Za Caribou"
author: "Clayton T. Lamb, PhD, UBCO"
...
rmarkdown::render('nutritional_hyp.Rmd',
...
```
##summarize and plot
```{r summarize and plot, results='hide', message=FALSE, warning=FALSE}
##summarize and plot
bod.cond%>%
...
```{r summary table, message=FALSE, warning=FALSE, results='hide'}
df%>%
  group_by(LOCCY)%>%
...
##Run GLMM Models
```{r glmm, results='hide', message=FALSE, warning=FALSE} 
df <- df%>%
...
###IS GLMM needed?
```{r glmm vs glm, results='hide', message=FALSE, warning=FALSE} 
m5l <- glm(SRCCY ~   LOCCY,
           data = df,
           family = "binomial")
...
##GLM
```{r glm, results='hide', message=FALSE, warning=FALSE} 
m1 <- glm(SRCCY ~  SRCLY, 
            data = df,
            family = "binomial")
...
```{r predict, results='hide', message=FALSE, warning=FALSE} 
...
##plot together
```{r plot together, fig.height=9, fig.width=10, message=FALSE, warning=FALSE, results='hide'}
...
##Another Approach- test the expected vs observed frequencies of observations
```{r expected vs obs, fig.height=9, fig.width=10, message=FALSE, warning=FALSE, results='hide'}
