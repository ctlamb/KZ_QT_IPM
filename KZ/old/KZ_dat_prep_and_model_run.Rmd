...
##Recruitment not converging
```{r effects, results='asis'}
conv <- data.frame(mean=out_rnd_eff$mean$R[,1],
                   lower=out_rnd_eff$q2.5$R[,1],
                   upper=out_rnd_eff$q97.5$R[,1],
                   yrs=yrs,
                   class="repro")

ggplot(conv, aes(x = yrs, y = mean, fill=class))+
  geom_ribbon(aes(ymin=lower, ymax=upper))+
  geom_line() +
  theme_ipsum()+
  labs(x="Year", y="Rate", title="Reproduction")
```